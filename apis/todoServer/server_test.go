...
func TestGet(t *testing.T) {
	...
	// httptestパッケージを使用してテスト用のserverインスタンス作成
		...
		t.Run(tc.name, func(t *testing.T) {
			var (
				resp struct {
					...
					TodoResults int       `json:"total_results"`
				}
				body []byte
				err  error
			)
			// テスト用serverに対してGETリクエスト
			...
			case strings.Contains(r.Header.Get("Content-Type"), "application/json"):
				...
				if resp.TodoResults != tc.expItems {
					t.Errorf("Expected %d items, got %d.", tc.expItems, resp.TodoResults)
				}
...
func TestAdd(t *testing.T) {
	...
	t.Run("Add", func(t *testing.T) {
		...
		// item structをJSONエンコードしたデータをbodyにセット
		if err := json.NewEncoder(&body).Encode(item); err != nil {
			t.Fatal(err)
		}

		// JSON化したitem(body)を/todoにPOST
